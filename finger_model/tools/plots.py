...
def animation(history, dt, name=None, history2=None, tendons=False):
    ...
    for i in tqdm.tqdm(range(0, N, di)):
        ...
        if tendons:
            ...
            ax.add_patch(patches.Circle((x, y), radius=RADII[J_MCP][T_FS], color=FS_COLOR, linewidth=0.1, fill=False))

            # PIP
            x, y = key_points[1, i], key_points[5, i]
            ax.add_patch(patches.Circle((x, y), radius=RADII[J_PIP][T_FS], color=FS_COLOR, linewidth=0.1, fill=False))
            pip_x, pip_y = rotate((x, y - RADII[J_PIP][T_FS]), (x, y), -(num.pi / 2 - angles[i, 1]))

            plt.plot([-0.1, mcp_x, pip_x], [-RADII[J_MCP][T_FS], mcp_y, pip_y], color=FS_COLOR)

            ### IO tendon ###
            # MCP
            x, y = key_points[0, i], key_points[4, i]
            mcp_x, mcp_y = rotate((x, y - RADII[J_MCP][T_IO]), (x, y), - (num.pi / 2 - angles[i, 0]))
            ax.add_patch(patches.Circle((x, y), radius=RADII[J_MCP][T_IO], color=IO_COLOR, linewidth=0.1, fill=False))

            # PIP
            x, y = key_points[1, i], key_points[5, i]
            ax.add_patch(patches.Circle((x, y), radius=RADII[J_PIP][T_IO], color=IO_COLOR, linewidth=0.1, fill=False))
            pip_x, pip_y = rotate((x, y + RADII[J_PIP][T_IO]), (x, y), -(num.pi / 2 - angles[i, 1]))

            plt.plot([-0.1, mcp_x, pip_x], [-RADII[J_MCP][T_IO], mcp_y, pip_y], color=IO_COLOR)

            ### FP tendon ###
            # MCP
            x, y = key_points[0, i], key_points[4, i]
            mcp_x, mcp_y = rotate((x, y - RADII[J_MCP][T_FP]), (x, y), - (num.pi / 2 - angles[i, 0]))
            ax.add_patch(patches.Circle((x, y), radius=RADII[J_MCP][T_FP], color=FP_COLOR, linewidth=0.1, fill=False))

            # PIP
            x, y = key_points[1, i], key_points[5, i]
            ax.add_patch(patches.Circle((x, y), radius=RADII[J_PIP][T_FP], color=FP_COLOR, linewidth=0.1, fill=False))
            pip_x, pip_y = rotate((x, y - RADII[J_PIP][T_FP]), (x, y), -(num.pi / 2 - angles[i, 1]))

            # DIP
            x, y = key_points[2, i], key_points[6, i]
            ax.add_patch(patches.Circle((x, y), radius=RADII[J_DIP][T_FP], color=FP_COLOR, linewidth=0.1, fill=False))
            dip_x, dip_y = rotate((x, y - RADII[J_DIP][T_FP]), (x, y), -(num.pi / 2 - angles[i, 1]))

            plt.plot([-0.1, mcp_x, pip_x, dip_x], [-RADII[J_MCP][T_FP], mcp_y, pip_y, dip_y], color=FP_COLOR)
            ...
            plt.plot([-0.1, mcp_x, pip_x, dip_x], [RADII[J_MCP][T_ED], mcp_y, pip_y, dip_y], color=ED_COLOR)
